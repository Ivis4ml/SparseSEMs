...
multiRegression = function(Xs, Ys, Sk, gamma, n, p, k, trans = FALSE) {
  ...
  fit = .Call("MultiReg", Xs, Ys, Sk, gamma, n, p, k, PACKAGE = "SparseSEMs")
  fit
}


## cross-validation on ridge regression
##' @title cv.multiRegression
...
cv.multiRegression = function(Xs, Ys, Sk, ngamma = 20, nfold = 5, n, p, k) {
  ...
  gamma_max = L2lamax(Xs, Ys, Sk, n, p, k)
  gamma_factors = 10 ** (seq(0, -4, length.out = ngamma)) * gamma_max
...
multiFSSEMiPALM = function(Xs, Ys, Bs, Fs, Sk, sigma2, lambda, rho,
  ...
  sigma2 = sigma2FSSEM(Xs, Ys, Bs, Fs, n, p, m)
  ...
  while (niter <= maxit) {
    ...
    if (converged || niter > maxit || sigma2 < 1e-6) {
      mu = lapply(1:m, function(k) {
        (diag(p) - Bs[[k]]) %*% mY[[k]] - sapply(1:p, function(i) { mX[Sk[[i]]] %*% Fs[[k]][i, Sk[[i]]] })
      })
      if (sparse) {
...
initLambdaiPALM = function(Xs, Ys, Bs, Fs, Sk, sigma2, Wl, Wf, p, k) {
  ...
  sigma2 = sigma2FSSEM(Xs, Ys, Bs, Fi, n, p, m)
  res = vector("list", m)
  for (j in 1:m) {
    res[[j]] = abs(1 / sigma2 * tcrossprod(Ys[[j]] - Fi[[j]] %*% Xs, Ys[[j]])) / Wl[[j]]
  }
  lambda_max = max(sapply(res, max))
  while (TRUE) {
    fit = multiFSSEMiPALM(Xs, Ys, Bs, Fs, Sk, sigma2, lambda = lambda_max, rho = 0, Wl, Wf, p = p,
...
initRhoiPALM = function(Xs, Ys, Bs, Fs, Sk, sigma2, Wl, Wf, lambda, n, p) {
  ...
  for(k in 1:m) {
    ImB = diag(p) - fit$Bs[[k]]
    d = n[k] * solve(ImB) - tcrossprod(ImB %*% Ys[[k]] - fit$Fs[[k]] %*% Xs - tcrossprod(fit$mu[[k]], rep(1, n[k])), Ys[[k]]) / fit$sigma2
...
##' @export
cv.multiFSSEMiPALM = function(Xs, Ys, Bs, Fs, Sk, sigma2, nlambda = 20, nrho = 20,
  ...
  lambda_max  = initLambdaiPALM(Xs, Ys, Bs, Fs, Sk, sigma2, Wl, Wf, p, q)
  lambda_factors = 10 ** seq(0, -3, length.out = nlambda) * lambda_max
  rho_factors = vector("list", nlambda)
  for (i in 1:nlambda) {
    lambda  = lambda_factors[i]
    rho_max = initRhoiPALM(Xs, Ys, Bs, Fs, Sk, sigma2, Wl, Wf, lambda, n, p)
  ...
  Ytest  = vector("list", nfold)
  for (i in 1:nfold) {
    Xtrain[[i]] = Xs[, cvfold != i, drop = F]
    Xtest[[i]]  = Xs[, cvfold == i, drop = F]
    Ytrain[[i]] = lapply(Ys, function(Y) {
      Y[, cvfold != i, drop = F]
    })
    Ytest[[i]]  = lapply(Ys, function(Y) {
  ...
  for (i in 1:nfold) {
    ...
    for (ilambda in 1:nlambda) {
      for (irho in 1:nrho) {
        ...
        if (j %% nrho == 1) {
          fit[[j]] = multiFSSEMiPALM(Xtrain[[i]], Ytrain[[i]], Bs, Fs, Sk, sigma2, lambda, rho, Wl, Wf, p,
                                     maxit = 50, threshold = 1e-5, sparse = FALSE, trans = TRUE, verbose = FALSE)
        } else {
          if (rho_factors[[ilambda]][irho] == rho_factors[[ilambda]][irho - 1]) {
            fit[[j]] = fit[[j-1]]
          } else {
            fit[[j]] = multiFSSEMiPALM(Xtrain[[i]], Ytrain[[i]], Bs, Fs, Sk, fit[[j-1]]$sigma2, lambda, rho, Wl, Wf, p,
                                     maxit = 50, threshold = 1e-5, sparse = FALSE, trans = TRUE, verbose = FALSE)
          }
        }
        err = logLiklihood(Xtest[[i]], Ytest[[i]], fit[[j]]$Bs, fit[[j]]$Fs, fit[[j]]$mu, fit[[j]]$Dets, fit[[j]]$sigma2, p)
        if (i == 1) {
...
## optimized model selection of FSSEM selected from BIC/eBIC
...
opt.multiFSSEMiPALM = function(Xs, Ys, Bs, Fs, Sk, sigma2, nlambda = 20, nrho = 20,
  ...
  for (ilambda in 1:nlambda) {
      for (irho in 1:nrho) {
        ...
        if (j %% nrho == 1) {
          fit[[j]] = multiFSSEMiPALM(Xs, Ys, Bs, Fs, Sk, sigma2, lambda, rho, Wl, Wf, p,
                                     maxit = 50, threshold = 1e-5, sparse = FALSE, trans = TRUE, verbose = FALSE)
        } else {
          if (rho_factors[[ilambda]][irho] == rho_factors[[ilambda]][irho - 1]) {
            fit[[j]] = fit[[j-1]]
        ...
        err = bayesianInfocriterion(Xs, Ys, fit[[j]]$Bs, fit[[j]]$Fs, fit[[j]]$mu, fit[[j]]$Dets, fit[[j]]$sigma2, p)
